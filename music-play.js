        ...
        for (let n = 0; n < DATA.length; n++) {
            for (let p = 0; p < DATA[n].song.length; p++) {
                ...
                if (DATA[n].song[p].songName == m) {
                    ...
                    var characterHTML = `
                        <li>
                            <div><img class="artistImg" style="width: 45px;height:45px;border-radius: 5px;" src="${DATA[n].song[p].songImage}" alt="">
        
                            </div>
                            <div class="artistContent">
                                <div style="cursor:pointer">${DATA[n].song[p].songName}</div>
                                <div class="follow">
                                    <div class="trackNum">${DATA[n].artist}<span><img src="pj-img/record.png" alt=""></span></div>
                                </div>
                            </div>
                        </li>
                    `;
                    if (content4.getElementsByTagName('li').length <4){
                        // console.log(songAddedCount)
                        content4.insertAdjacentHTML('beforeend', characterHTML);
                        console.log(content4.getElementsByTagName('li'));
                    }else {
                        console.log(content4.getElementsByTagName('li')[3]);
                        content4.removeChild(content4.getElementsByTagName('li')[3]);
                        content4.insertAdjacentHTML('afterbegin', characterHTML);
                    }           
                    // songAddedCount++;
                }
            }
